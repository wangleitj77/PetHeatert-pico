...
def syncTime():
    ...
    rtc = RTC()
    rtc.datetime((int(g_startAt[:4]),int(g_startAt[5:7]),int(g_startAt[8:10]),0
                  ,int(g_startAt[11:13]),int(g_startAt[14:16]),int(g_startAt[17:19]),0))
    

config = readConfig()
#startServer()
g_cn = Control(config)
g_ps = Pserver(config, g_cn)
g_ps.startWifiServer()
syncTime()
